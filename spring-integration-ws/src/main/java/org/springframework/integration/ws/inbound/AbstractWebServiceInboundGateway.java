...
public abstract class AbstractWebServiceInboundGateway extends MessagingGatewaySupport
	...
	public void invoke(MessageContext messageContext) throws Exception {
		...
		try {
			...
			Exception exceptionToCheck = ex;
			while ((exceptionToCheck instanceof MessagingException || exceptionToCheck instanceof ExpressionException)
					&& exceptionToCheck.getCause() instanceof Exception exception) {
	...
	protected void toSoapHeaders(WebServiceMessage response, Message<?> replyMessage) {
		if (response instanceof SoapMessage soapMessage) {
			this.headerMapper.fromHeadersToReply(replyMessage.getHeaders(), soapMessage);
		}
	}
