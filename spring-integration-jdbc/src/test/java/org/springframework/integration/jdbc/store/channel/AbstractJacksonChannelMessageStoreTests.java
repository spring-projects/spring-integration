...
public abstract class AbstractJacksonChannelMessageStoreTests {
	...
	protected static final String REGION = "JSON_TEST_REGION";
	...
	@BeforeEach
	public void init() {
		this.messageStore = new JdbcChannelMessageStore(this.dataSource);
		this.messageStore.setRegion(REGION);
		this.messageStore.setChannelMessageStoreQueryProvider(this.queryProvider);
		this.messageStore.setPreparedStatementSetter(
				new JacksonChannelMessageStorePreparedStatementSetter(getTrustedPackages()));
	...
	public void testCustomPayloadSerialization() {
		...
		assertThat(retrievedPayload.getSubject()).isEqualTo("Order Confirmation");
		assertThat(retrievedPayload.getBody()).isEqualTo("Your order has been confirmed.");
		assertThat(retrievedPayload.getTo()).isEqualTo("customer@example.com");
	...
	@Test
	public void testJsonIsHumanReadableInDatabase() throws Exception {
		TestMailMessage payload = new TestMailMessage(
				"Test Subject",
				"Test Body",
				"test@example.com"
		...
		String storedJson = jdbcTemplate.queryForObject(
				"SELECT MESSAGE_BYTES FROM INT_CHANNEL_MESSAGE WHERE MESSAGE_ID = ? AND REGION = ?",
				String.class,
				messageId.toString(),
				REGION
		);

		String expectedJson = buildExpectedJson(messageId, timestamp, payload);

		JSONAssert.assertEquals(expectedJson, storedJson, JSONCompareMode.LENIENT);
	}
	...
	public void testJsonContainsAllMessageHeaders() {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(this.dataSource);
		String storedJson = jdbcTemplate.queryForObject(
				"SELECT MESSAGE_BYTES FROM INT_CHANNEL_MESSAGE WHERE MESSAGE_ID = ?",
				String.class,
	...
	private String buildExpectedJson(UUID messageId, Long timestamp, TestMailMessage payload) {
		// @formatter:off
		return "{\n" +
				"  \"@class\": \"org.springframework.messaging.support.GenericMessage\",\n" +
				"  \"payload\": {\n" +
				"    \"@class\": \"org.springframework.integration.jdbc.store.channel.TestMailMessage\",\n" +
