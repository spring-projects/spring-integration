 ...
 * <p>
 * The {@link StandardIntegrationFlow} implementation (produced by {@link Builder})
 ...
 *
 *        &#64;Override
 *        public void configure(IntegrationFlow.Definition&lt;?&gt; f) {
...
public interface IntegrationFlow {
	 ...
	 * configure an integration flow with the provided {@link Definition}.
	 * @param flow the {@link Definition} to configure
	 */
	void configure(Definition<?> flow);
	 ...
	 * application context.
	 *
	 * @param <B> the {@link Definition} implementation type.
	 *
	 * @author Artem Bilan
	 * @author Gary Russell
	 ...
	 * @since 5.1
	 *
	 * @see IntegrationFlowBeanPostProcessor
	 */
	abstract class Definition<B extends Definition<B>> {
		 ...
		 * @return the current {@link Definition}.
		 */
		 ...
		 * the {@link IntegrationFlowBeanPostProcessor}
		...
		private MessageChannel obtainInputChannelFromFlow(IntegrationFlow flow) {
			...
			if (messageChannel == null) {
				...
				Definition<?> flowBuilder = IntegrationFlows.from(messageChannel);
		...
		/**
		 * Populate the {@link MethodInvokingRouter} for the {@link MessageProcessor}
			...
			if (!CollectionUtils.isEmpty(componentsToRegister)) {
				for (Map.Entry<Object, String> entry : componentsToRegister.entrySet()) {
					...
					if (component instanceof Definition) {
						Definition<?> flowBuilder = (Definition<?>) component;
	 ...
	 * @since 5.1
	 */
	final class Builder extends Definition<Builder> {
