...
public class MessagingAnnotationPostProcessorChannelCreationTests {
	...
	public void testAutoCreateChannel() {
		...
		willAnswer(invocation -> invocation.getArgumentAt(0, DirectChannel.class))
				.given(beanFactory).initializeBean(any(DirectChannel.class), eq("channel"));
		willAnswer(invocation -> invocation.getArgumentAt(0, MessageHandler.class))
				.given(beanFactory).initializeBean(any(MessageHandler.class),
				eq("foo.foo.serviceActivator.handler"));
