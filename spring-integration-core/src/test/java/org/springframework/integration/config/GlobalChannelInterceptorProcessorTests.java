...
public class GlobalChannelInterceptorProcessorTests {
	...
	@Before
	public void setup() {
		globalChannelInterceptorProcessor = new GlobalChannelInterceptorProcessor();
		beanFactory = mock(ListableBeanFactory.class);
		globalChannelInterceptorProcessor.setBeanFactory(beanFactory);
	}

	@Test
	public void testProcessorWithNoInterceptor() {
		when(beanFactory.getBeansOfType(GlobalChannelInterceptorWrapper.class))
				.thenReturn(Collections.emptyMap());
		globalChannelInterceptorProcessor.afterSingletonsInstantiated();
		verify(beanFactory).getBeansOfType(GlobalChannelInterceptorWrapper.class);
		verify(beanFactory, Mockito.never()).getBeansOfType(ChannelInterceptorAware.class);
	}
	...
	public void testProcessorWithInterceptorDefaultPattern() {
		...
		GlobalChannelInterceptorWrapper globalChannelInterceptorWrapper = new GlobalChannelInterceptorWrapper(
				channelInterceptor);
		...
		when(beanFactory.getBeansOfType(GlobalChannelInterceptorWrapper.class)).thenReturn(interceptors);
		when(beanFactory.getBeansOfType(ChannelInterceptorAware.class)).thenReturn(channels);
		globalChannelInterceptorProcessor.afterSingletonsInstantiated();
