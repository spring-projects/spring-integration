    ...
    public static class ProtoConfig {
        ...
        public IntegrationFlow flow3() {
            return IntegrationFlow.from(in3())
                    .transform(new ToProtobufTransformer())
                    .wireTap(tapped())
                    .enrichHeaders(h -> h.header(ProtoHeaders.TYPE, TestClass2.class.getName(), true))
                    .transform(fromTransformer())
        ...
        public IntegrationFlow flow5() {
            return IntegrationFlow.from(in5())
                    .transform(new ToProtobufTransformer().typeExpression("'protoTest'"))
                    .wireTap(tapped())
                    .transform(new FromProtobufTransformer(TestClass1.class)
                            .typeExpression("'protoTest' == headers[proto_type] ? '"
        ...
        public IntegrationFlow flow6() {
            return IntegrationFlow.from(in6())
                    .transform(new ToProtobufTransformer().typeExpression("'wontFindThisHeader'"))
                    .wireTap(tapped())
                    .transform(new FromProtobufTransformer(TestClass1.class)
                            .typeExpression("'protoTest' == headers[proto_type] ? '"
                                    + TestClass2.class.getName() + "' : null"))
