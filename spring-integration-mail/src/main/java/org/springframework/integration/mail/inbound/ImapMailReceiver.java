...
public class ImapMailReceiver extends AbstractMailReceiver {
	...
	/**
	 * Provide a way to set custom {@link SearchTermStrategy} to compile a {@link SearchTerm}
	...
	public void cancelPing() {
		ScheduledFuture<?> pingTaskToCancel = this.pingTask;
		if (pingTaskToCancel != null) {
			pingTaskToCancel.cancel(true);
		}
		closeFolder();
	}
	...
	/**
	 * Retrieve new messages from this receiver's folder. This implementation
	...
	private Message[] nullSafeMessages(@Nullable Message[] messageArray) {
	...
	private class DefaultSearchTermStrategy implements SearchTermStrategy {
		...
		public @Nullable SearchTerm generateSearchTerm(Flags supportedFlags, Folder folder) {
			...
			boolean recentFlagSupported = supportedFlags.contains(Flag.RECENT);
			if (recentFlagSupported) {
				searchTerm = new FlagTerm(new Flags(Flag.RECENT), true);
			}
			if (supportedFlags.contains(Flag.ANSWERED)) {
				NotTerm notAnswered = new NotTerm(new FlagTerm(new Flags(Flag.ANSWERED), true));
				if (searchTerm == null) {
			...
			if (supportedFlags.contains(Flag.DELETED)) {
				NotTerm notDeleted = new NotTerm(new FlagTerm(new Flags(Flag.DELETED), true));
				if (searchTerm == null) {
			...
			if (supportedFlags.contains(Flag.SEEN)) {
				NotTerm notSeen = new NotTerm(new FlagTerm(new Flags(Flag.SEEN), true));
				if (searchTerm == null) {
					searchTerm = notSeen;
				}
				else {
