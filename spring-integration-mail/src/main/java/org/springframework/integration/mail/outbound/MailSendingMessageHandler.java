...
public class MailSendingMessageHandler extends AbstractMessageHandler {
	...
	private MailMessage createMailMessageFromByteArrayMessage(Message<byte[]> message) {
		...
		String attachmentFileName = headers.get(MailHeaders.ATTACHMENT_FILENAME, String.class);
		...
		Integer multipartMode = headers.get(MailHeaders.MULTIPART_MODE, Integer.class);
	...
	private static String @Nullable [] retrieveHeaderValueAsStringArray(MessageHeaders headers, String key) {
		...
		if (value != null) {
			if (value instanceof @Nullable String[] strArr) {
