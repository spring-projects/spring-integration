...
public abstract class HttpRequestHandlingEndpointSupport extends MessagingGatewaySupport
	 ...
	 * to override the '200 OK' default one.
	 * <p> The {@link #statusCodeExpression} is applied only for one-way {@code <http:inbound-channel-adapter/>}.
	...
	protected void onInit() throws Exception {
		...
		if (this.statusCodeExpression != null) {
			this.statusCodeEvaluationContext = createEvaluationContext();
		}
	}
	...
	protected void setStatusCodeIfNeeded(ServletServerHttpResponse response) {
		if (this.statusCodeExpression != null) {
			Object value = this.statusCodeExpression.getValue(this.statusCodeEvaluationContext);
