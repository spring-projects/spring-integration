...
class Mqtt3MessageDrivenChannelAdapterTests implements HiveMQContainer {
	...
	@BeforeEach
	void setUp() {
		var builder = MqttClientBuilderHelper.clone(mqtt3ClientBuilder);
		mqtt3TestClient = builder.identifier("mqtt5-test-client").buildBlocking();
	...
	static class Config {
		...
		@Bean
		Mqtt3MessageDrivenChannelAdapter carDeviceInboundChannelAdapter(Mqtt3ClientBuilder mqtt3ClientBuilder,
				QueueChannel carDeviceOutputChannel) {
			var adapter = new Mqtt3MessageDrivenChannelAdapter(mqtt3ClientBuilder, CAR_DEVICE_TOPIC_WILDCARD);
		...
		@Bean
		Mqtt3MessageDrivenChannelAdapter petDeviceInboundChannelAdapter(Mqtt3ClientBuilder mqtt3ClientBuilder,
				QueueChannel petDeviceOutputChannel, Executor executor) {
			var adapter = new Mqtt3MessageDrivenChannelAdapter(mqtt3ClientBuilder, PET_DEVICE_TOPIC);
