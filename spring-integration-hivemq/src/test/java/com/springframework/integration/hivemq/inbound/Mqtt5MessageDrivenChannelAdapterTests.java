...
class Mqtt5MessageDrivenChannelAdapterTests implements HiveMQContainer {
	...
	void testCarDeviceWildcardTopic() throws InterruptedException {
		...
		// Given
		mqtt5AsyncClient.publishWith().topic(CAR_DEVICE_TOPIC_1)
				...
				.join();
		mqtt5AsyncClient.publishWith().topic(CAR_DEVICE_TOPIC_2)
	...
	void testPetDeviceTopic() throws InterruptedException {
		...
		Mqtt5Publish mqtt5Publish = new MqttPublishBuilder.Default()
				...
				.build();
		mqtt5AsyncClient.publish(mqtt5Publish)
				.orTimeout(10000, TimeUnit.MILLISECONDS)
				.join();
		// When
	...
	static class Config {
		...
		@Bean
		Mqtt5MessageDrivenChannelAdapter carDeviceInboundChannelAdapter(Mqtt5AsyncClient mqtt5AsyncClient,
				QueueChannel carDeviceOutputChannel, SmartMessageConverter mqttStringToBytesConverter) {
			var adapter = new Mqtt5MessageDrivenChannelAdapter(mqtt5AsyncClient, CAR_DEVICE_TOPIC_WILDCARD);
		...
		@Bean
		Mqtt5MessageDrivenChannelAdapter petDeviceInboundChannelAdapter(Mqtt5AsyncClient mqtt5AsyncClient,
				QueueChannel petDeviceOutputChannel, Executor executor) {
			var adapter = new Mqtt5MessageDrivenChannelAdapter(mqtt5AsyncClient, PET_DEVICE_TOPIC);
