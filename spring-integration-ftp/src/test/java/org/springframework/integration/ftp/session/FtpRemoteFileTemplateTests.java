...
public class FtpRemoteFileTemplateTests {
	...
	public void testINT3412AppendStatRmdir() {
		...
		template.executeWithClient(new ClientCallback<FTPClient, Void>() {

			@Override
			public Void doWithClient(FTPClient client) {
				try {
		...
		template.execute(new SessionCallback<FTPFile, Void>() {

			@Override
			public Void doInSession(Session<FTPFile> session) throws IOException {
