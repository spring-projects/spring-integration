...
* **Note** When you rebased a branch which has been pushed to remote already, you have to use `git push origin --force` to apply the latest changes to the Pull Request.
This doesn't apply to the `upstream master` because fo history overriding in case of `--force`.
