...
Starting with _version 4.3_, the HTTP module provides an `@EnableIntegrationGraphController` `@Configuration`-level annotation and `<int-http:graph-controller/>` XML component, respectively, to expose `IntegrationGraphServer` as a REST service.
