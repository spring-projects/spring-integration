...
This component is similar to the MVC-based `HttpRequestHandlingEndpointSupport` and has with it some common options via newly extracted `BaseHttpInboundEndpoint`, but can be configured, installed and performed in the Reactive foundation - Spring WebFlux environment.
...
Everything looks similar to the `HttpRequestHandlingEndpointSupport` mentioned above, except that we use `@EnableWebFlux` to add WebFlux infrastructure to our integration application.
Also `ReactiveHttpInboundEndpoint` performs `sendAndReceive` operation to the downstream flow via back-pressure, on demand based manner provided by the reactive HTTP server implementation.

NOTE: The reply part is non-blocked as well and based on the internal `FutureReplyChannel` which is flat-mapped to the reply `Mono` for on demand resolution.

The `ReactiveHttpInboundEndpoint` can be configured with the custom `ServerCodecConfigurer`, `RequestedContentTypeResolver` and even `ReactiveAdapterRegistry`.
The last one let us to return a reply as any reactive type - Reactor `Flux`, RxJava `Observable`, `Flowable` etc.
This way we can simply implement https://en.wikipedia.org/wiki/Server-sent_events[Server Sent Events] scenario with the Spring Integration components:
...
The `ReactiveHttpRequestExecutingMessageHandler` (starting with _version 5.0_) implementation is very similar to `HttpRequestExecutingMessageHandler` instead of delegating to a `WebClient` from Spring Framework WebFlux module.
