...
To configure for this situation, do not configure the `multipartResolver` bean, configure the endpoint to expect
a `byte[]` request, and configure it with a single `MessageConverter` of type `MultipartPassThroughHttpMessageConverter`:
...
<bean id="converter" class="org.springframework.integration.http.converter.MultipartPassThroughHttpMessageConverter" />
----
====
