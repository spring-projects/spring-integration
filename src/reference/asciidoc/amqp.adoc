...
property.
If this option isn't specified, too, the `AmqpInboundGateway` will rely on the provided `AmqpTemplate` during the send
operation or throws `IllegalStateException` for the internal `AmqpTemplate` which is exactly without default
