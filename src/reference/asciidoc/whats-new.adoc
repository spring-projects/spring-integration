...
The `GlobalChannelInterceptor` annotation's `pattern` attribute now supports negative patterns.

You can now pass negative patterns a leading `!` to negate a pattern(`'!foo*'` means don't export components
where the name matches the pattern `'foo*'`).
