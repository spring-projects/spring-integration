...
A new `IntegrationGraphServer` together with the `IntegrationGraphController` REST service are provided to expose runtime model of Spring Integration components as a graph.
