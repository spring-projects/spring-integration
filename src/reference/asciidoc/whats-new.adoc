...
A new inbound channel adapter is provided that returns an `InputStream` for each file.
See <<sftp-streaming>> for more information.
...
A new inbound channel adapter is provided that returns an `InputStream` for each file.
See <<ftp-streaming>> or more information.
