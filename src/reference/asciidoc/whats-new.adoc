...
Also a reactive implementation for MongoDb change stream support is present with a `MongoDbChangeStreamMessageProducer`.
...
The `MessageProducerSupport` now has a `subscribeToPublisher(Publisher<? extends Message<?>>)` API to allow to implement message-driven producer endpoints which emit messages via reactive `Publisher`.
