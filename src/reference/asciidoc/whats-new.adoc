...
The `FileReadingMessageSource` doesn't check and create a directory now until its `start()` is called.
So, if Inbound Channel Adapter for the `FileReadingMessageSource` is `autoStartup = false`, there is no failures against file system during application start up.
