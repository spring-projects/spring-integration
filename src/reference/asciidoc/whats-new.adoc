...
* <<x5.1-aggregator>>
* <<x5.1-publisher>>
* <<x5.1-file-writing-message-handler-new-file-callback>>
...
If you are using `FileExistsMode.APPEND` or `FileExistsMode.APPEND_NO_FLUSH` you can provide a `newFileCallback` that will be called when creating a new file. This callback receives the newly
created file and the message that triggered the callback. This could be used to write a CSV header for an example.
