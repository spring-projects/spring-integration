...
According a new `InvocableHandlerMethod`-based infrastructure for service method invocations, now we can perform `contentType` conversion from payload to target method argument.
...
The one of the appropriate service methods can now be marked with the `@Primary` to allow fallback for non-matched conditions.
