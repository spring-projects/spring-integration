...
The `JdbcPollingChannelAdapter` can now be provided with a `Supplier<?>` for the select and update queries, allowing the queries to change for each poll.
See <<./jdbc.adoc#jdbc-inbound-channel-adapter,JDBC Inbound Channel Adapter>> for more information.
