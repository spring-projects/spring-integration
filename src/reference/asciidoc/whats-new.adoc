...
[[x5.1-rateLimitAdvice]]
=== Rate Limit Advice Support

One more handler advice `RequestHandlerRateLimiterAdvice` is now available to rate limit requests on handlers for a given time unit.
See <<rateLimiter-advice>> for more information.