...
==== UDP outbound adapter socket-expression

New `socket-expression` option for outbound UDP packet-sending Channel Adapter was introduced. This option enables the outbound
channel adapter to use other adapters datagram socket. It expects proper SpEL expression describing which external datagram
socket to use.
