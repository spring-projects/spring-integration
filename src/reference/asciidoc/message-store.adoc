...
The value of this option can be anything what could be parsed later on for any reason to make a decision for the group.
...
The `MessageGroupStore` expose a `setGroupCondition(Object groupId, String condition)` API.
...
This function is evaluated against each message to be added to the group and existing condition on the group.
The implementation may decide to return new value, existing or reset the target condition to `null`.
...
The `AbstractCorrelatingMessageHandler` checks provided `ReleaseStrategy` for this contract implementation and extracts a `conditionSupplier` for further group condition evaluation logic.
