...
<6> A comma-separated list of message header names which should not be be populated into `Message` during headers copying operation.
...
By default only `MessageHeaders.ID` and `MessageHeaders.TIMESTAMP` aren't copied during message building.
