...
Starting with _version 4.3.10_, the `ErrorMessagePublisher` together with the `ErrorMessageStrategy` are provided.
The former can be used as general purpose of errors publishing.
Can be called or extended in any error handling or retry scenarios.
In fact, the `ErrorMessageSendingRecoverer` extends this class for the `RecoveryCallback` purpose.
The `ErrorMessageStrategy` is used to build `ErrorMessage` based on the provided exception and `AttributeAccessor` context.
It can be injected to any `MessageProducerSupport` and `MessagingGatewaySupport` - and `requestMessage` is stored under `ErrorMessageUtils.INPUT_MESSAGE_CONTEXT_KEY` in the `AttributeAccessor` context.
The `ErrorMessageStrategy` can use that `requestMessage` as an `originalMessage` property of the `ErrorMessage` to publish.
