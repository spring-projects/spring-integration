...
The service activator is essentially a generic endpoint for calling method on some object with an input message and its headers.
Its internal logic is based on a `MessageHandler` which can be any possible implementation for specific use-case, for example `DefaultMessageSplitter`, `AggregatingMessageHandler`, `SftpMessageHandler`, `JpaOutboundGateway` etc.
