...
Starting with _version 5.0_, the service method can be marked with the `@Primary` as a fallback for all non-matching cases.
This can be use-full in case of <<content-type-conversion>>, when we would like to end-up with some fallback method for non-matched conditions.
