...
The service method can return any type which becomes as a reply message payload.
In this case a new `Message<?>` object is created and all the headers from a request message are copied.
This works the same way for most Spring Integration `MessageHandler` implementation, when interaction is based on the POJO method invocation.

A full `Message<?>` object can be returned from the method as well.
However need to keep in mind that, unlike <<transformer>>, in case of Service Activator this message is going to be modified for copying the headers from a request message if they not present in the returned message already.
So, if you try to perform some existing headers removal in your service method, they are going to appear in the reply message back.
This, essentially, not a Service Activator responsibility to remove headers for reply message and pursuing the loosely-coupling principle it is a proper way to pair with a `HeaderFilter` in the integration flow.
The Transformer can be used in a Service Activator role, but returning a full `Message<?>` from method call is already an end-user responsibility and you need to ensure the proper headers population for a reply message, including copying request message headers.
