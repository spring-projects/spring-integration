...
If you are not interested in a reply producing based on the service method return, you should configure `output-channel` as a `nullChannel`.
See <<channel-special-channels>> for more information.

The Service Activator is one of those components who doesn't require a return for producing reply.
If your method returns `null` or it is just `void`, the Service Activator stops its work after method invocation without any signals.
This behaviour can be controlled by the `AbstractReplyProducingMessageHandler.requiresReply` option.
