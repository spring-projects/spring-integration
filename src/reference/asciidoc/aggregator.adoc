...
Since _version 5.0_, `AbstractCorrelatingMessageHandler` keeps track of the groupIds being processed by the handler and will only expire groups which are handled by the handler, this will be helpful for cases when same `MessageStore` is shared by multiple handlers.
...
Ensure proper `CorrelationStrategy` is used when same `MessageStore` is used for multiple aggregators to ensure unique correlationkey is being assigned to the messages.
