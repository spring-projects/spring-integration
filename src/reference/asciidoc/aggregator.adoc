...
This functionality can be controlled by newly introduced `popSequenceDetails` `boolean` property, so the `MessageBuilder.popSequenceDetails()` can be disabled in some scenarios when correlation details have not been populated by the standard splitter.
