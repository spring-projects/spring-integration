...
Starting with _version 4.3_, the `WireTap` has two more constructors for just `channelName`.
It is convenient for Java Configuration and when we actively use channel auto-creation logic.
The target `MessageChannel` bean is resolved from the provided `channelName` later, on the first interaction with the
interceptor.
This _late-binding_ approach also allows to simplify the typical wire-tapping patterns with Java DSL configuration:
[source,java]
----
@Bean
