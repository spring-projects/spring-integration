...
A `FixedSubscriberChannel` is a variant of a `DirectChannel` with only single `MessageHandler` to subscribe provided during initialization.
This is useful for high-throughput performance use-cases when no other subscribers are involved and no channel interceptors needed.
...
The `FluxMessageChannel` is a `org.reactivestreams.Publisher` implementation for `"sinking"` sent messages into an internal `reactor.core.publisher.Flux` for on demand consumption by the reactive subscribers downstream.
This channel implementation is nor a `SubscribableChannel`, neither `PollableChannel`, so only `org.reactivestreams.Subscriber` instances can be used for this channel consumption honoring back-pressure nature of reactive streams.
On the other hand, the `FluxMessageChannel` implements a `ReactiveStreamsSubscribableChannel` with its `subscribeTo(Publisher<Message<?>>)` contract to allow to accept events from reactive source publishers bridging a reactive stream into the integration flow.
To achieve a fully reactive behavior for the whole integration flow, such a channel must be placed between all the endpoints in the flow.

See <<./reactive-streams.adoc#reactive-streams,Reactive Streams Support>> for more information about an interaction with Reactive Streams.
...
The Java configuration variant is about a `DirectChannel` bean definition:
