...
Starting with _version 4.3_, the outbound gateway has a second constructor that takes a `ProxyFactoryPostProcessor` bean as a second argument.
...
Starting with _version 5.0_, this can be set using the XML namespace, using the `post-processor` attribute.
