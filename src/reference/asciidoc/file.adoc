...
NOTE: Starting with version 5.5, the `FileInboundChannelAdapterSpec` of Java DSL has a convenient `recursive(boolean)` option to use the mentioned `RecursiveDirectoryScanner` in the target `FileReadingMessageSource` instead of default one.
