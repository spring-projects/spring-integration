...
It uses an internal queue for 'eligible files' as a result of `scanner`.
The `scanEachPoll` option is used to make sure the internal queue is refreshed with the latest content of the input
directory on each poll.
By default (`scanEachPoll = false`) the `FileReadingMessageSource` empties its queue before looking at the directory again.
In cases where order is relevant it is important to consider the effects of setting this flag to `true`.
However the default behaviour is particularly useful to reduce scans of large numbers of files in a directory.
From other side `FileReadingMessageSource` can accept the `Comparator<File>` as a constructor argument to allow the
internal queue (`PriorityBlockingQueue`) reorder its content according to the business requirements.
Therefore there is no reason to implement ordering logic in the custom `DirectoryScanner`.
