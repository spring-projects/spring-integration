...
Previously there was no simple way to prevent an operation system permissions error when we would like do not deal with directory, for example from tests or when permissions are applied later.
