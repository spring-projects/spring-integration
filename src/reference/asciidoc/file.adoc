...
<8> The name for the header to carry the first line of the content as message header for the rest of lines.
Since _version 5.0_.
...
Starting with _version 5.0_, the `firstLineAsHeader` option is introduced to represent the first line of content under the provided header name for the messages of the rest of lines.
This line isn't included to the sequence and the `FileMarker.END` has `lineCount` without this specific header.
If file contains only the header line, the file is treated as empty and therefore only `FileMarker` s are emitted during splitting.
By default there is no header extraction.
