...
It is also important to understand that filters (including `patterns`, `regex`, `prevent-duplicates` etc) and `lockers`,
...
For the case of an external `scanner` all filters and lockers attributes are prohibited on the
`FileReadingMessageSource` in favor of their options on that custom `DirectoryScanner`.
In other words, if you inject `scanner` into the `FileReadingMessageSource`, you should supply `filter` and `locker`
on that `scanner` not `FileReadingMessageSource`.

NOTE: The `DefaultDirectoryScanner` uses `IgnoreHiddenFileListFilter` and `AcceptOnceFileListFilter` by default.
So, to prevent them you should configure your own filter (e.g. `AcceptAllFileListFilter`) or even configure it to `null`.
