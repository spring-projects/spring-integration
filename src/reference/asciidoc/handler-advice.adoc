...
* `RequestHandlerRateLimiterAdvice` (described in <<rateLimiter-advice>>)
...
The `o.s.i.handler.advice.ExpressionEvaluatingRequestHandlerAdvice` supplied advice class is more general than the other two advices.
...
The `RequestHandlerRateLimiterAdvice` advice supports following properties: `rate`, `maxDelay` and `timeUnit`.
The `rate` property represents the maximum number of requests per period to throttle.
This option must be provided as a positive number.
The `maxDelay` property represents the maximum time the thread will wait.
If the thread wait time exceeds the `maxDelay` time, `RateLimiterMaxDelayException` exception will be thrown.
Default value for `maxDelay` is -1 which means no max delay.
The `timeUnit` property represents the unit of time `java.util.concurrent.TimeUnit`.
This option must be non null.

The following example configures a RateLimiter advice.
...
<int:service-activator input-channel="input" ref="failer" method="service">
    <int:request-handler-advice-chain>
        <bean class="o.s.i.handler.advice.RequestHandlerRateLimiterAdvice">
            <property name="rate" value="60" />
            <property name="maxDelay" value="3" />
...
In the preceding example, the `rate` is set to `60`, `maxDelay` is set to `3` and `timeUnit` is set to `MINUTES`.
With this configurations 60 requests will be allowed per minute with max delay of 3 minutes.
