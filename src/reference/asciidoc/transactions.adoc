...
Spring Integration 5.0 introduces `PassthroughTransactionSynchronizationFactory` which is applied by default to pollers
 when no `TransactionSynchronizationFactory` is configured but a TX advice of type `TransactionInterceptor` exists.
 Using `PassthroughTransactionSynchronizationFactory` pollers can bind messages to a transactional
context and provide the failedMessage in the `MessagingException` if an exception is thrown in the TX advice.
When using a a custom TX advice `PassthroughTransactionSynchronizationFactory` must be configured explicitly to
