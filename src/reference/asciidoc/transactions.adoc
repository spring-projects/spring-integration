...
Spring Integration 5.0 introduces `PassthroughTransactionSynchronizationFactory` which is applied for pollers if there
is no `TransactionSynchronizationFactory` available and the TX Advice is an instance of `TransactionInterceptor`.
A `PassthroughTransactionSynchronizationFactory` can also be explicitly configured if you are using a a custom TX
Advice.
