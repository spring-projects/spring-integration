...
Spring integration leverages the https://www.springsource.org/spring-gemfire[Spring Gemfire] project, providing a thin wrapper over its components.
...
The stored procedure support leverages  Spring's https://docs.spring.io/spring/docs/3.0.x/javadoc-api/org/springframework/jdbc/core/simple/SimpleJdbcCall.html[`SimpleJdbcCall`] class and consequently supports stored procedures for:
