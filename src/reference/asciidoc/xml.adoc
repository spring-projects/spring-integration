...
Starting with `4.2 version` the `XPathMessageSplitter` exposes `outputProperties`
...
Starting with `4.2 version` the `XPathMessageSplitter` exposes an `iterator` mode option as `boolean` flag (defaults
to `true`). This allows to "stream" splitted nodes in the downstream flow. For example in the `iterator` mode
each node is transformed while iterating, whereas the `list` contains already transformed nodes. See <<splitter>> for
more information.
