...
[IMPORTANT]
=====
The `log()` or `wireTap()` opearators are applied to the current `MessageChannel` (if it is an instane of `ChannelInterceptorAware`) or an intermediate `DirectChannel` is injected into the flow for the currently configured endpoint.
In the example above the `WireTap` interceptor is added to the `myChannel` directly, because `DirectChannel` implements `ChannelInterceptorAware`:
[source,java]
----
@Bean
...
When current `MessageChannel` doesn't implement `ChannelInterceptorAware`, an implicit `DirectChannel` and `BridgeHandler` are injected into the `IntegrationFlow` and only after that a `WireTap` is added to this new `DirectChannel`.
And when there is no any channel declaration like in this sample:
[source,java]
----
.handle(...)
.log()
