...
Starting with version 6.1, the `IntegrationFlowDefinition` expose a convenient `handleReactive(ReactiveMessageHandler)` terminal operator.
Any `ReactiveMessageHandler` implementation (even just plain lambda with `Mono` API) can be used for this operator.
...
In addition, a `ReactiveMessageHandlerSpec`-based variants also provided.
