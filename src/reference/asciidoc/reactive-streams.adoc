...
In case of standard imperative `MessageChannel` use-case and if a reply payload is a *multi-value* publisher (see `ReactiveAdapter.isMultiValue()` for more information), it is wrapped into a `Mono.just()`.
A result of this `Mono` has to be subscribed explicitly downstream or flattened by the `FluxMessageChannel`, but still downstream.
With a `ReactiveStreamsSubscribableChannel` for `outputChannel` not need to worry about return type and subscription: everything is processed smoothly by the framework internally.
...
The `FluxMessageChannel` is a combined implementation of the `MessageChannel` and `Publisher<Message<?>>`.
A `Flux`, as a hot source, is created internally for sinking incoming message from `send()` implementation.
...
Events from this delegating publishers are sank into an internal `Flux` mentioned above.

A consumer for the `FluxMessageChannel` must be a `org.reactivestreams.Subscriber` instance for honoring Reactive Streams contract.
Fortunately all the `MessageHandler` implementations in Spring Integration also implement a `CoreSubscriber` from project Reactor.
...
The flow behavior is change in this case, of course: from imperative, push model to the reactive pull one.
...
A polling trigger is built from the provided options and used for periodic scheduling for task to poll a target source of data or events.
...
This generator `Flux` is subscribed them by the provided `ReactiveStreamsSubscribableChannel` honoring a back-pressure downstream.
...
When an `AbstractMessageSplitter` gets a `Publisher` for its logic, the process goes naturally over items in `Publisher` to map them into messages for producing into an `outputChannel`.
If this channel is a `ReactiveStreamsSubscribableChannel`, the `Flux` wrapper for the `Publisher`is subscribed on demand from that channel and this splitter behavior looks more like a `flatMap` Reactor's operator, when we map an incoming event into multi-value output `Publisher`.
Especially, it makes more sense when the whole integration flow is built with a `FluxMessageChannel` before and after splitter, aligning Spring Integration configuration with a Reactive Streams requirements and its operators for event processing.
With a regular channel, a `Publisher` is converted into an `Iterable` for standard iterate-and-produce splitting logic.

A `FluxAggregatorMessageHandler` is another sample of particular Reactive Streams logic implementation which could be treated as a `"reactive operator"` in terms of Project Reactor.
It is fully based on the `Flux.groupBy()` and `Flux.window()` (or `buffer()`) operators.
The incoming messages are sank into a `Flux.create()` initiated when a `FluxAggregatorMessageHandler` is created making it as a hot source.
This `Flux` is subscribed by the `ReactiveStreamsSubscribableChannel` on demand or directly in the `FluxAggregatorMessageHandler.start()` when `outputChannel` is not reactive.
...
An `IntegrationFlow` in Java DSL can start from any `Publisher` instance (see `IntegrationFlows.from(Publisher<Message<T>>)`), as well as with an `IntegrationFlowBuilder.toReactivePublisher()` operator, the `IntegrationFlow` can be turned into a reactive hot source.
A `FluxMessageChannel` is used internally in both cased: it can subscribe to inbound `Publisher` according its `ReactiveStreamsSubscribableChannel` contract and it is a `Publisher<Message<?>>` by itself for downstream subscribers.
...
For exactly opposite use-case, when `IntegrationFlow` should call a reactive stream and continue after completion, a `fluxTransform()` operator is provided in the `IntegrationFlowDefinition`.
