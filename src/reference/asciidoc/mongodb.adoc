...
Starting with version 5.3, `spring-integration-mongodb` modules introduces a `MongoDbChangeStreamMessageProducer` - a reactive `MessageProducerSupport` implementation for Spring Data `ReactiveMongoOperations.changeStream(String, ChangeStreamOptions, Class)` API.
This component produces a `Flux` of messages with a `body` of `ChangeStreamEvent` as a payload by default and some change stream related headers (see `MongoHeaders`).
It is recommended to combine this `MongoDbChangeStreamMessageProducer` with a `FluxMessageChannel` as an `outputChannel` for on demand subscription and events consumption downstream.
...
When the `MongoDbChangeStreamMessageProducer` is stopped, or subscription is cancelled downstream, or MongoDb change stream produces an `OperationType.INVALIDATE`, the `Publisher` is completed.
...
This channel adapter can be reconfigured for new options in between if there is a requirement to consume change stream events from other places.
