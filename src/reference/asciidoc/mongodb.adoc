...
Starting with _version 5.0_, the MongoDb Outbound Gateway allows you to query a database by sending a Message to its request channel.
The gateway will then send the response to the reply channel.
The Message payload and headers can be used to specify the query.
...
* `query` or `query-expression` - Specifies the MongoDb query. Please refer to http://www.mongodb.org/display/DOCS/Querying[MongoDB documentation] for more query samples.
...
public class MongoDbJavaApplication {
    ...
    public MessageHandler mongoDbOutboundGateway() {
        ...
        gateway.setCollectionNameExpression(new LiteralExpression("foo"));
        gateway.setMongoDbQuery("{'name' : 'Bob'}");
    ...
    @Bean
    @ServiceActivator(inputChannel = "replyChannel", poller =  @Poller(fixedDelay = "2000"))
...
public class MongoDbJavaApplication {
    ...
    private MongoDbOutboundGatewaySpec queryOutboundGateway() {
        return MongoDb.outboundGateway(this.mongoDbFactory, this.mongoConverter)
                .query("{name : 'Bob'}")
                .collectionNameExpression(new LiteralExpression("foo"))
