...
Any Security for the `IntegrationGraphController` can be achieved with the standard configuration options and components provided by the Spring Security project.

The `graph-controller` element has an attribute for configuring Cross Origin restrictions; this simply allows pages from the provided origins to access the `path` with the `GET` method.
For more sophisticated configuration you can use the standard Spring MVC project configuration options and components.

A simple example of that may be:
...
<int-http:graph-controller path="/myIntegration" allowed-origins="http://localhost:9090" />
----
