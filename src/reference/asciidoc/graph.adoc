...
The version 5.2.2 has introduced an `IntegrationPattern` abstraction and all out-of-the-box components, which belongs to particular EI pattern implements this abstraction and provides an `IntegrationPatternType` enum value.
This information can be useful for some categorizing logic in the target application or being exposed into the graph node it can be used on UI to determine how to draw the provided component.
