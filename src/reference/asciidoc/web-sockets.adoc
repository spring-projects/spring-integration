...
When we destroy a dynamic flow registration, an associated `ServerWebSocketContainer` instance is destroyed, too, as well as respective endpoint registration including URL path mappings.