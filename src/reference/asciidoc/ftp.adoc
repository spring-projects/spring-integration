...
On the other hand some servers return an empty result for `NLIST` command, when path exists and it is a directory.
...
- `NLIST` - Perform the `NLIST` FTP command - `FTPClient.listName(path)`;
- `NLIST_AND_DIRS` -  Perform the `NLIST` command first and if it fails fall back to `FTPClient.changeWorkingDirectory(path)`.
See `FtpSession.exists()` for more information.

Since we know that the `FileExistsMode.FAIL` case is always only looking for a file (and not a directory), we safely use `NLIST` mode as a default one for `FtpMessageHandler` and `FtpOutboundGateway` components on the internal `FtpRemoteFileTemplate` instance.
