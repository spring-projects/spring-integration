...
For that purpose, `` (the base class for all FTP Session Factories) exposes hooks, in the form of the two post-processing methods below.
...
Starting with _Spring Integration version 4.2_, the `MessageSessionCallback<F, T>` implementation can be used to instantiate
 the `<int-ftp:outbound-gateway/>` (`FtpOutboundGateway`) to perform any operation(s) on the `Session<FTPFile>` with
 the `requestMessage` context.
It can be used for any non-standard or low-level operation (or several at once) with the FTP, allowing to get a gain
from integration flow definition from one side, and _functional_ interface (Lambda) implementation injection from
another:
...
For the XML configuration variant the `<int-ftp:outbound-gateway/>` provides a `session-callback` attribute to allow to
...
From Java perspective they are required from different constructor in the `FtpOutboundGateway` class.
