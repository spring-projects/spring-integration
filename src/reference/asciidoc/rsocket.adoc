...
Starting with version 5.2.1, the `ServerRSocketMessageHandler` is extracted to the public, top-level class for possible connection with an existing RSocket server.
When a `ServerRSocketConnector` is supplied with an external instance of `ServerRSocketMessageHandler`, it doesn't create an RSocket server internally and just delegate all the handling logic to the provided instance.
In addition the `ServerRSocketMessageHandler` can be configured with a `messageMappingCompatible` flag to handle also `@MessageMapping` for RSocket controller replacing fully a functionality provided by the standard `RSocketMessageHandler`.
This can be useful in mixed configurations, when classical `@MessageMapping` methods present in the same application alongside with RSocket channel adapters and externally configured RSocket server is present in the application.
