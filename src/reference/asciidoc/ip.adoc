...
| socket-expression
| SpEL expression
| A SpEL expression to be evaluated to determine which datagram socket use for sending outgoing UDP packets (e.g. UDP inbound packet receiving Channel Adapter socket can be used).
Cannot be used together with options `host`, `port`, `acknowledge`, `multicast`.
|===
