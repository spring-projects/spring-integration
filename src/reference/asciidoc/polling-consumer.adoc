...
With these message sources, the `ACKNOWLEDGMENT_CALLBACK` header (see <<message-header-accessor>>) is added to the message.
...
The callback for the `poll` method is a `MessageHandler`.
