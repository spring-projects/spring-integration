...
The `FromProtobufTransformer` should specify either the expect class type explicitly (use the `setExpectedType` method) or use a SpEL expression to determine the type to deserialize using the `setTypeExpression` method.
