...
Starting with _version 4.2_, the `@SecuredChannel` annotation is available in favor of deprecated
`ChannelSecurityInterceptorFactoryBean`, which has been introduced since _version 4.0_ for the Java & Annotation
configuration in the `@Configuration` classes. The `ChannelSecurityInterceptorFactoryBean` has been deprecated to
avoid some undesired early load for dependant beans from the `BeanFactory` during `ApplicationContext` initialization
phase.

With the `@SecuredChannel` annotation the Java configuration variant of the XML config above looks like:
