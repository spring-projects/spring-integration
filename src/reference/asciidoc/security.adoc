...
<int:channel id="processChannel">
    ...
    <int:interceptors>
        ...
        <bean class="org.springframework.integration.security.context.SecurityContextCleanupChannelInterceptor"/>
    </int:interceptors>
</int:channel>
...
configuration or using `<request-handler-advice-chain>` on the Spring Integration component:
