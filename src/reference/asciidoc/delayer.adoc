...
If the `maxAttempts` is reached, the message is discarded (unless the release is transactional, in which case the message will remain in the store, but will no longer be scheduled for release, until the application is restarted).
...
The `ErrorMessage` contains a header `IntegrationMessageHeaderAccessor.DELIIVERY_ATTEMPT` containing the current count.
