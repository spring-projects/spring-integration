...
Starting with version 5.3.7, if transaction is active when message is stored into a `MessageStore`, the release task is scheduled in a `TransactionSynchronization.afterCommit()` callback.
This is necessary to ensure that there is no a race condition when message is not stored because no transaction commit yet, but scheduled task is ready to be performed after delay.
