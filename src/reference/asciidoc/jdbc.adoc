...
Starting with version 6.0.5, specifying a `PlatformTransactionManager` on `PostgresSubscribableChannel` will notify subscribers in a transaction.
...
Starting with version 6.0.5, a retry policy can be specified by providing a `RetryTemplate` to `PostgresSubscribableChannel`.
