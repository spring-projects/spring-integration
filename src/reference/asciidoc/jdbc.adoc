...
Starting with _version 5.0_, the  JDBC-based `MetadataStore` (<<metadata-store>>) implementation is available.
...
In order to instruct these adapters to use the `JdbcMetadataStore`, simply declare a Spring bean using the
...
The `INT_` can be changed according to the target data base design requirements and `JdbcMetadataStore` can be configured to the custom prefix.

The `JdbcMetadataStore` also implements `ConcurrentMetadataStore`, allowing it to be reliably shared across multiple
application instances where only one instance will be allowed to store or modify a key's value.
All these operations are _atomic_ via transaction guarantees.

The transaction management is required to use `JdbcMetadataStore`.
All the Inbound Channel Adapters can be supplied with the `TransactionManager` reference for their poller configuration.
Unlike non-transactional `Metadata` store implementations, with `JdbcMetadataStore`, the entry appears in the target table only after transaction commit.
In case of rollback, no entries is added to the `INT_METADATA_STORE`.