...
There are some cases when the flexibility and loosely-coupling of `SqlParameterSourceFactory` isn't enough for the target
`PreparedStatement` or we need to do some low-level JDBC work.
The Spring JDBC module provides enough API to configure the execution environment (e.g. `ConnectionCallback`
or `PreparedStatementCreator`) and manipulate with parameter values (e.g. `SqlParameterSource`).
Or even API for low level operations, for example with `StatementCallback`.

From this perspective starting with _Spring Integration 4.2_ the `MessagePreparedStatementSetter` is available to allow
to specify parameters on the `PreparedStatement` manually being in the `requestMessage` context.
This class plays exactly the same role as `PreparedStatementSetter` in the standard Spring JDBC API.
Actually it is invoked directly from the inline `PreparedStatementSetter` implementation, when `JdbcMessageHandler`
performs its executing on the `JdbcTemplate`.

This functional interface option is mutually exclusive with `sqlParameterSourceFactory` and can be used as a more
powerful alternative to populate parameters to the `PreparedStatement` from the `requestMessage`.
...
`<int-jdbc:outbound-channel-adapter>` component, respectively, to specify a  `MessagePreparedStatementSetter`
bean reference.
...
It allows to specify a `MessagePreparedStatementSetter` bean reference, which implements more sophisticated
