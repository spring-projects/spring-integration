...
If you need to use a single MQTT ClientID for several integrations, you cannot use different MQTT client instances because MQTT brokers may have a limitation on a number of connections per ClientID (typically, a single connection is allowed).
For having a single client reused for different channel adapters, you may use a `org.springframework.integration.mqtt.core.ClientManager` component and pass it to any channel adapter you need.
It will manage MQTT connection lifecycle and do automatic reconnects if needed.
You can also provide a custom connection options and `MqttClientPersistence` just as you would do it for channel adapter components.
...
@Bean
public IntegrationFlow mqttInFlowFoo(
        ClientManager<IMqttAsyncClient, MqttConnectionOptions> clientManager) {

    Mqttv5PahoMessageDrivenChannelAdapter messageProducer =
        new Mqttv5PahoMessageDrivenChannelAdapter(clientManager, "foo-topic");
    return IntegrationFlow.from(messageProducer)
            .channel(c -> c.queue("fromMqttChannel"))
            .get();
}
...
@Bean
public IntegrationFlow mqttInFlowBar(
        ClientManager<IMqttAsyncClient, MqttConnectionOptions> clientManager) {

    Mqttv5PahoMessageDrivenChannelAdapter messageProducer =
        new Mqttv5PahoMessageDrivenChannelAdapter(clientManager, "bar-topic");
    return IntegrationFlow.from(messageProducer)
            .channel(c -> c.queue("fromMqttChannel"))
            .get();
}
...
IMPORTANT: You should use a single client manager instance so that the connection is reused.
Ensure you do not create it for each channel adapter.