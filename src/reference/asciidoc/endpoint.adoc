...
For this purpose the framework registers a `SmartLifecycleRoleController` bean in the application context with the `IntegrationContextUtils.INTEGRATION_LIFECYCLE_ROLE_CONTROLLER` name.
Whenever it is necessary to control lifecycles, this bean can be injected:
