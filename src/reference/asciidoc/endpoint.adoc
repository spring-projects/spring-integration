...
Its `HandlerMethodArgumentResolver` implementations (e.g. `PayloadArgumentResolver` and `MessageMethodArgumentResolver`) can use `MessageConverter` abstraction to convert an incoming `payload` to the target method argument type.
The conversion can be based on the `contentType` message header.
For this purpose Spring Integration provides `ConfigurableCompositeMessageConverter` that delegates to a list of registered converters to be invoked until one of them returns a non-null result.
By default this converter provides (in that strict order):
...
The `ConfigurableCompositeMessageConverter` is called like that because it can be be supplied with any other `MessageConverter` s including or excluding mentioned above default converters
and registered as an appropriate bean in the application context overriding default one:
...
----
And those two new converters will be registered in the composite before defaults.
You can even don't use this `ConfigurableCompositeMessageConverter`, but provide your own `MessageConverter` and register bean with the `IntegrationContextUtils.ARGUMENT_RESOLVER_MESSAGE_CONVERTER_BEAN_NAME` name.

NOTE: The `MessageConverter`-based (including `contentType` header) conversion isn't available in case of SpEL method invocation, because this mechanism is very close to the regular Java code flow, especially when we use compiled SpEL.
In this case we get a gain only from the regular class to class conversion mentioned above in the <<payload-type-conversion>>.
