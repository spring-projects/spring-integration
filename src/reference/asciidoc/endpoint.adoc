...
To participate in a leader election and be notified when elected leader, when leadership is revoked or, failure to acquire the resources to become leader,
an application creates a component in the application context called a "leader initiator".
...
Users can also receive failure notifications by setting the `publishFailedEvents` to `true`, in cases when they want take a specific action if a failure occurs.
