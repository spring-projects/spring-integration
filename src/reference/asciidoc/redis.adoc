...
IMPORTANT: The `task-executor` has to be configured for more than one thread for processing, otherwise there is a possible dead lock when `RedisQueueMessageDrivenEndpoint` tries to restart listener task in case of error.
The `errorChannel` can be used to swallow those errors to avoid restarts, but it preferable do not put your application to the dead lock situation.
...
IMPORTANT: The `task-executor` has to be configured for more than one thread for processing, otherwise there is a possible dead lock when `RedisQueueInboundGateway` tries to restart listener task in case of error.
The `errorChannel` can be used to swallow those errors to avoid restarts, but it preferable do not put your application to the dead lock situation.
See Spring Framework https://docs.spring.io/spring/docs/current/spring-framework-reference/integration.html#scheduling-task-executor-types[Reference Manual] for possible `TaskExecutor` implementations.
