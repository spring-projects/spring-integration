...
At the time of writing, the shade plugin does not merge the `spring.factories` files; you just get one which appears to be the first one encountered by the plugin when it extracts the jar contents.
This mostly defeats the Spring Integration bootstrapping mechanism so you will need to take further action to ensure the framework is configured properly.

The following are two examples of how to work around this issue.

**Manually fix `spring.factories`**
...
Edit the `META-INF/spring.factories` file to include initializers for all required modules

As of 5.0, the `-core` `spring.factories` looks like this:
...
And the `-jmx` file looks like this:
...
- `jar cf ../myAppFixed.jar .`


**Or, add a `BeanFactoryPostProcessor` `@Bean`, to perform the same functions as the initializers**
...
public static BeanFactoryPostProcessor dslInitializer() {
    return new BeanFactoryPostProcessor() {
        ...
        public void postProcessBeanFactory(ConfigurableListableBeanFactory bf) throws BeansException {
            // core
            new GlobalChannelInterceptorInitializer().initialize(bf);
            new IntegrationConverterInitializer().initialize(bf);
            new IdempotentReceiverAutoProxyCreatorInitializer().initialize(bf);
            new DslIntegrationConfigurationInitializer().initialize(bf);
            // groovy
            new GroovyIntegrationConfigurationInitializer().initialize(bf);
            // http
            new HttpIntegrationConfigurationInitializer().initialize(bf);
            // jmx
            new JmxIntegrationConfigurationInitializer().initialize(bf);
            // security
            new SecurityIntegrationConfigurationInitializer().initialize(bf);
            // webflux
            new WebFluxIntegrationConfigurationInitializer().initialize(bf);
            // websocket
            new WebSocketIntegrationConfigurationInitializer().initialize(bf);
            // ws
            new WsIntegrationConfigurationInitializer().initialize(bf);
        }
...
Obviously, you would only include the initializers for the modules you are using in your application.

IMPORTANT: https://docs.spring.io/spring-boot/docs/1.5.10.RELEASE/reference/html/executable-jar.html[Spring Boot's executable jar mechanism] takes a different approach in that it nests the jars, thus retaining each `spring.factories` file on the class path. So, with a Spring Boot application, this work around is not needed.
