...
IMPORTANT: Starting with _version 4.0_, the `@EnableIntegration` annotation has been introduced, to allow register
Spring Integration infrastructure (see its http://docs.spring.io/spring-integration/docs/latest-ga/api/org/springframework/integration/config/EnableIntegration.html[JavaDocs]).
This annotation is required when only Java & Annotations configuration is used, e.g. with Spring Boot and/or
Spring Integration Messaging Annotations support and Spring Integration Java DSL.

The `@EnableIntegration` is also useful when you have a parent context with no Spring Integration components and 2 or more child contexts that do use Spring Integration.
It would enable these common components to be declared once only, in the parent context.

The `@EnableIntegration` registers many infrastructure components with the application context:

- Register some built-in beans, e.g. `errorChannel` and its `LoggingHandler`, `taskScheduler` for pollers,
`jsonPath` SpEL-function etc.;
- Add several `BeanFactoryPostProcessor` s to enhance `BeanFactory` for global and default integration environment;
- Add several `BeanPostProcessor` s toi enhance and/or convert and wrap particular beans for integration purposes;
- Add annotations processors to parse Messaging Annotations and register components for then with the application
context.
...
Also see the <<annotations>> for more information about Messaging Annotations.
...
With the XML configuration and Spring Integration Namespace support it is hidden by the Parsers infrastructure how
target beans are built and wired together.
For Java & Annotations Configuration it is important to understand the Framework API for the target end-user
applications.

The first class citizens for EIP implementation are `Message`, `Channel` and `Endpoint` (see <<overview-components>> above).
And their implementations (contracts) are:
...
When two first are enough simple to understand and they are things per se from the implementation, configuration and
usage perspective, the last one deserves more review.

The `AbstractEndpoint` is widely used throughout the Framework for different components implementations, in most cases
for inbound gateways, but its main implementations are:

- `EventDrivenConsumer`, when we subscribe to the `SubscribableChannel` to _listen_ for messages;
- `PollingConsumer`, when we _poll_ for messages from the `PollableChannel`.

Using Messaging Annotations and/or Java DSL you shouldn't worry about these components, because Framework produces them
automatically via appropriate annotations and `BeanPostProcessor` s.
With manual components building the `ConsumerEndpointFactoryBean` should be used to help to determine the target
`AbstractEndpoint` implementation basing by the provided `inputChannel` property.

From other hand the `ConsumerEndpointFactoryBean` exhibit an another first class citizens in the Framework -
...
protocol outbound adapters are its implementations, too, e.g. `FileWritingMessageHandler`,
`HttpRequestExecutingMessageHandler`, `AbstractMqttMessageHandler` etc.
When you develop Spring Integration application with Java & Annotations Configuration, you should take a look into the
Spring Integration module to find an appropriate `MessageHandler` implementation to be used for the `@ServiceActivator`
configuration.
For example to send an XMPP message (<<xmpp>>) we should configure something like this:
...
The _inbound_ message flow side has its own components, which divided to the _polling_ and _listening_ behavior.
The listening components are pretty simple and typically requires only one target class implementation to be ready to
produce messages.
The listening components can be one-way, the `MessageProducerSupport` implementations,
...
The _polling_ inbound endpoints are for those protocols which don't provide the listener API or aren't intended for
such a behavior.
For example any File based protocol, as an FTP, any data bases (RDBMS or NoSQL) etc.

These inbound endpoints consist with two components: the poller environments, to initiate the polling task periodically,
and message source class to read data from the target protocol and produce message for the downstream integration flow.
The first class, for poller configuration, is `SourcePollingChannelAdapter`.
It is one more `AbstractEndpoint` implementation, but especially for the polling purpose on the integration flow beginning.
...
exactly `MessageSource` interface, e.g. `MongoDbMessageSource` and `AbstractTwitterMessageSource`.
With that in hands our config for the reading data from the RDBMS table wit JDBC may look like:
...
- `o.s.i.websocket.inbound.WebSocketInboundChannelAdapter` - the `MessageProducerSupport`
...
If you are familiar with Spring Integration XML configuration already, starting with the _version 4.3_ we provide in the
