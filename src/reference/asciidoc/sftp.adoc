...
Starting with _Spring Integration version 4.2_, the `MessageSessionCallback<F, T>` implementation can be used to instantiate
 the `<int-sftp:outbound-gateway/>` (`SftpOutboundGateway`) to perform any operation(s) on the
 `Session<ChannelSftp.LsEntry>` with the `requestMessage` context.
It can be used for any non-standard or low-level operation (or several at once) with the SFTP, allowing to get a gain
...
For the XML configuration variant the `<int-sftp:outbound-gateway/>` provides a `session-callback` attribute to allow
...
From Java perspective they are required from different constructor in the `SftpOutboundGateway` class.
