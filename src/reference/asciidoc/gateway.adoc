...
The reason that the attributes on the 'gateway' element are named 'default-request-channel' and 'default-reply-channel'
is that you may also provide per-method channel references by using the `@Gateway` annotation.
...
To accomplish this, our Gateway provides support for a Message Channel dedicated to the errors via the _error-channel_ attribute.
...
NOTE: If the downstream flow returns an `ErrorMessage`, its `payload` (an `Exception`) is treated as a regular downstream
 error: if there is an `error-channel` configured, it will be sent there, to the error flow; otherwise it is thrown
 to the caller of gateway.
The same re-throw process is done for the `ErrorMessage` returned from the error flow on the `error-channel`.
This can be useful in async situations when we need propagate `Exception` directly to the caller, bypassing any
error handling.
To achieve this goal there is just enough to return an `Exception` as a `reply` from some service, instead of throwing it.
...
sample demonstrates this technique to emulate Socket IO error to the waiting thread using `aggregator` `group-timeout` (see <<agg-and-group-to>>)
