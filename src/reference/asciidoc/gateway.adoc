...
Of course, unlike the `Future<?>` return types, there is no way to inform the caller if some exception is thrown by the flow (although an `ErrorMessage` will be sent to the error channel, if configured).
