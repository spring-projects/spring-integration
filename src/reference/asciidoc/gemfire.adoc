...
The GemFire https://gemfire-82-javadocs.docs.pivotal.io/com/gemstone/gemfire/cache/EntryEvent.html[EntryEvent] is the `#root` object of the `expression` evaluation.
Example:
[source]
----
...
Please consult the https://gemfire.docs.pivotal.io/97/geode/developing/continuous_querying/chapter_overview.html[GemFire documentation] for more information on implementing continuous queries.
...
In Spring Integration, the MessageStore strategy also provides the foundation for the https://www.enterpriseintegrationpatterns.com/patterns/messaging/StoreInLibrary.html[ClaimCheck] pattern, which is described in EIP as well.
