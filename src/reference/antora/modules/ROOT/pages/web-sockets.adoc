...
IMPORTANT: The dynamic Websocket endpoints can only be registered via a Spring Integration mechanism: when regular Spring `@EnableWebsocket` is used, Spring Integration configuration backs off and no infrastructure for dynamic endpoints is registered.
