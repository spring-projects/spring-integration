...
[[http-java-serialization]]
=== Java Serialization

The `SerializingHttpMessageConverter` is not registered by default.
To accept request bodies with the `application/x-java-serialized-object` content type, add this converter to the `inbound-channel-adapter` or the `inbound-gateway` by using the `message-converters` attribute (or the `messageConverters()` option of the Java DSL).

WARNING: Deserializing Java objects from untrusted sources is dangerous.
A crafted request body can chain classes available on the application classpath (so-called deserialization gadgets) to run arbitrary code while the object graph is being restored.
Consider using a data format such as JSON for the endpoints exposed to untrusted clients.
See the https://cheatsheetseries.owasp.org/cheatsheets/Deserialization_Cheat_Sheet.html[OWASP Deserialization Cheat Sheet] for more information.

The `SerializingHttpMessageConverter` deserializes request bodies through an `AllowListDeserializingConverter`.
Starting with version 7.2, use the `SerializingHttpMessageConverter(String... allowedPatterns)` constructor to specify the trusted classes.
This constructor requires at least one pattern and rejects an empty pattern array.
A pattern is either a fully qualified class name or a name with a `pass:[*]` wildcard at the beginning or at the end (for example, `pass:[com.example.model.*]` or `pass:[*.OrderDto]`).
The basic types (`String`, `Number` and its subclasses, primitives and arrays) are always allowed.
Deserialization fails for any other class that does not match one of the patterns.

IMPORTANT: The default constructor and `setAllowedPatterns()` method are deprecated for removal since version 7.2.
For backward compatibility, the deprecated default constructor still allows all classes when no patterns are configured.
Passing `"*"` to the new constructor explicitly allows all classes; use it only when all sources are trusted.
Always restrict the allowed patterns when the converter reads requests from untrusted clients.

The following example configures an inbound channel adapter to deserialize only classes from the `com.example.model` package:
...
public IntegrationFlow httpInboundFlow() {
    SerializingHttpMessageConverter converter =
            new SerializingHttpMessageConverter("com.example.model.*");
    return IntegrationFlow
            .from(Http.inboundChannelAdapter("/orders")
...
----
<int-http:inbound-channel-adapter id="ordersInbound" channel="orders"
    path="/orders"
...
Additional patterns can be added with the `addAllowedPatterns()` method.
The same allowlist applies when the converter reads a response body for an HTTP outbound endpoint.
...
See <<http-java-serialization>> for the security implications of Java deserialization.
=====
...
For much sophisticated scenarios it is recommended to configure a `UriBuilderFactory` on the externally provided `RestClient.Builder` (or an `UriTemplateHandler` on a `RestTemplate`, which is deprecated); or in case of WebFlux - `WebClient` with it `UriBuilderFactory`.
