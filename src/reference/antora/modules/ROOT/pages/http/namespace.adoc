...
The `SerializingHttpMessageConverter` deserializes request bodies through an `AllowListDeserializingConverter`, so you can restrict the classes to trust by configuring its `allowedPatterns`.
...
IMPORTANT: For backward compatibility, all classes are deserialized when no patterns are configured.
Always configure the allowed patterns when the converter reads requests from untrusted clients.
...
public IntegrationFlow httpInboundFlow() {
    ...
    converter.setAllowedPatterns("com.example.model.*");
...
<util:list id="converters">
    <bean class="org.springframework.integration.http.converter.SerializingHttpMessageConverter">
        <property name="allowedPatterns" value="com.example.model.*"/>
    </bean>
</util:list>
----
======
