...
The `rest-template` attribute is deprecated but remains available for backward compatibility.
...
If you do not explicitly configure the `request-factory`, `rest-template`, or `rest-client` attribute, a default `RestClient` (which uses a `SimpleClientHttpRequestFactory`) is instantiated.
