...
IMPORTANT: Since Control Bus is powerful enough to make changes into the system state, it is recommended to secure the `/control-bus` endpoints (for example, with Spring Security) and expose the Control Bus Controller only into DMZ.
