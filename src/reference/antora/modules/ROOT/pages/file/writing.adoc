...
WARNING: If the `MessageFlushPredicate` is based on regular expressions (like default one), the https://owasp.org/www-community/attacks/Regular_expression_Denial_of_Service_-_ReDoS[Regular expression Denial of Service (ReDoS)] vulnerability should be taken into account, when the trigger action for the `FileWritingMessageHandler` is exposed as an external endpoint accepting those regular expressions from untrusted sources.
