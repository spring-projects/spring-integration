...
Use the `Grpc` factory to add the `GrpcInboundGateway` to flows using the xref:dsl.adoc[].
[source,java]
----
@Bean
...
[[grpc-outbound-method-name-configuration]]
...
Use the `Grpc` factory to add the `GrpcOutboundGateway` to flows using the xref:dsl.adoc[].
