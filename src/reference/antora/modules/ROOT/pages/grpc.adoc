...
Use the `Grpc` factory to add the `GrpcInboundGateway` to flows using the xref:changes-5.0-5.1.adoc#x5.1-java-dsl[DSL]:
...
Add the `GrpcOutboundGateway` to flows using the DSL:
