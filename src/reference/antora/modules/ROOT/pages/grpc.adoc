...
----
+
By default, if no method name or expression is configured and the service has more than one method, the gateway uses the `GrpcHeaders.SERVICE_METHOD` header to determine which method to invoke.
...
* **Unary** methods accept a single gRPC request message and perform a blocking call, returning the gRPC response message directly.
