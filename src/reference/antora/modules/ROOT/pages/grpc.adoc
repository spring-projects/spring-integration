...
* **Unary**: Single request → `Mono` with single response
...
* **Unary** methods accept a single gRPC request message , returning a `Mono<ResponseType>` containing the response.
...
+
Client streaming methods return a `Mono<ResponseType>`, while bidirectional streaming methods return a `Mono<ResponseType>` containing the response.
