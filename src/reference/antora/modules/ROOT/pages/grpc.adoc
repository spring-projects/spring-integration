...
Use the `Grpc` factory to add the `GrpcInboundGateway` to flows using the xref:dsl.adoc[DSL].
The DSL provides a fluent API that simplifies configuration and improves readability.
The inbound gateway acts as a server-side endpoint that exposes your integration flow as a gRPC service.
...
The method name to invoke can be configured in four ways:
...
[[configuration-grpc-outbound-gateway-with-dsl]]
=== Configuration with DSL

Use the `Grpc` factory to add the `GrpcOutboundGateway` to flows using the xref:dsl.adoc[DSL].
The DSL provides a fluent API that simplifies configuration and improves readability.
...
For services with multiple methods, specify the method name expression or method name explicitly as shown below:
...
IntegrationFlow grpcOutboundFlow(ManagedChannel channel) {
	return f -> f
            .handle(Grpc.outboundGateway(channel, TestHelloWorldGrpc.class)
                    .methodName("SayHello"))
            .transform(this::upperCase);
}
...
----
NOTE: If neither a method name nor a method name expression is configured, and the service offers multiple methods, the gateway will look for the `GrpcHeaders.SERVICE_METHOD` header in the input message to determine which method to invoke.