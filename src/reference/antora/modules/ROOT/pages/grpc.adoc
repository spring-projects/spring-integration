...
The method name to invoke can be specified via an `Expression` using the `setMethodNameExpression()` setter.
If not specified, the gateway automatically detects the method name when the service descriptor contains exactly one method.
...
* **Unary**: Single request → single response (blocking call returns the message directly)
* **Server streaming**: Single request → `Flux` of multiple responses
* **Client streaming**: `Flux` of requests → `Mono` with single response
* **Bidirectional streaming**: `Flux` of requests → `Flux` of multiple responses

The following example demonstrates how to configure a `GrpcOutboundGateway` with an explicit method name:
...
public GrpcOutboundGateway grpcOutboundGateway(ManagedChannel channel) {
    ...
    gateway.setMethodNameExpression(new LiteralExpression("SayHello"));
    return gateway;
}
----

For services with a single method, the gateway can auto-detect the method name:
...
* For **Server streaming** methods, the gateway returns a `Flux<ResponseType>` containing all streamed responses.
* For **Client streaming** methods, the request payload must be a `Flux<RequestType>`, and the gateway returns a `Mono<ResponseType>` with the single aggregated response.
* For **Bidirectional streaming** methods, the request payload must be a `Flux<RequestType>`, and the gateway returns a `Flux<ResponseType>` with multiple responses.

=== Method Name Configuration

The method name can be specified in several ways:

* **Explicit method name**: Use `setMethodNameExpression()` with a `LiteralExpression` for a fixed method name
* **Auto-detection**: Omit the method name expression for services with exactly one method
...
public GrpcOutboundGateway dynamicMethodGateway(ManagedChannel channel) {
    GrpcOutboundGateway gateway = new GrpcOutboundGateway(channel, TestHelloWorldGrpc.class);
    // Method name determined from message header at runtime
