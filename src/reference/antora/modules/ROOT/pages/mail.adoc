...
@Bean
fun mailOutboundFlow(outboundMail: MessageChannel, mailSender: JavaMailSender): IntegrationFlow =
    IntegrationFlows.from(outboundMail)
        .handle(Mail.outboundAdapter(mailSender))
        .get()
----
Groovy DSL::
...
When configuring an inbound channel adapter, choose between polling or event-driven (assuming the mail server supports IMAP `idle` -- if not, then polling is the only option).
...
When encountering a requirement to filter incoming messages (for example, a requirement to read only emails that have 'Spring Integration' in the `Subject` line).
...
Transaction synchronization for inbound adapters enables different actions after a transaction commits or rolls back.
