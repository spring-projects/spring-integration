...
----
@Bean
IntegrationFlow mailOutboundFlow(JavaMailSender mailSender) {
    return IntegrationFlow.from("outboundMail")
        .handle(Mail.outboundAdapter(mailSender))
        .get()
}
...
You can also use a `header-enricher` message transformer.
...
fun mailOutboundFlow(outboundMail: MessageChannel) = integrationFlow(outboundMail) {
    ...
    handle(Mail.outboundAdapter(mailProperties.host)
        .port(mailProperties.port)
        .credentials(mailProperties.username, mailProperties.password)
        .javaMailProperties { p -> p.put("mail.smtp.starttls.enable", "true") })
}
----
Groovy DSL::
...
[[configuring-inbound-channel-adapters]]
== Configuring Inbound Channel Adapters
...
----
@Bean
fun imapMailInboundFlow(javaMailProperties: Properties, receiveChannel: MessageChannel): IntegrationFlow {
    return integrationFlow(
...
fun kotlinImapIdleFlow(receiveChannel: MessageChannel, mailMessageHandler: MailMessageHandler,
    javaMailProperties: Properties): IntegrationFlow {

    val receiver = ImapMailReceiver("imaps://[username]:[password]@imap.gmail.com/INBOX").apply {
...
IntegrationFlow groovyImapIdleFlow(MessageChannel receiveChannel, MailMessageHandler mailMessageHandler,
    ...
    def receiver = new ImapMailReceiver("imaps://[username]:[password]@imap.gmail.com/INBOX")      .with {
...
----
@Bean
fun createJavaMailProperties(): Properties = Properties().apply {
...
Properties createJavaMailProperties() {
    new Properties().with {
        it['mail.imaps.socketFactory.class'] = 'javax.net.ssl.SSLSocketFactory'
        it['mail.imaps.socketFactory.fallback'] = 'false'
...
ImapMailReceiver imapMailReceiver(SearchTermStrategy searchTermStrategy) {
    ...
    // ...
    receiver.searchTermStrategy = TestSearchTermStrategy()
    return receiver
}
----
XML::
...
Now, if you explicitly set this property to `false`, the message is marked as `\Seen` regardless of the setting of `shouldMarkMessagesRead`.
...
When using an IMAP `idle` channel adapter, connections to the server may be lost, (for example, through network failure), it is important to understand the JavaMail API and how to deal with them when configuring IMAP `idle` adapters.
Spring Integration mail adapters were tested with JavaMail 2.0.2.
You must pay special attention to some JavaMail properties that need to be set with regard to auto-reconnect.
...
This means that, when your POP3 mail adapter runs, emails are successfully consumed as they become available during each poll and no single email message is delivered more than once.
...
In other words, there are two valid and mutually exclusive uses that make it very hard to pick the single best default.
...
In some scenarios such a flag on the message in mailbox is not desirable, regardless `\Recent` or user flag is not supported as well.
...
Can there be duplication of messages to be filtered? In other words, if, on the last retrieval where you had five new messages and only one passed the filter, what happens with the other four?
...
public IntegrationFlow imapIdleFlow(ImapMailReceiver imapMailReceiver, MessageChannel receiveChannel,
	...
	ExpressionEvaluatingTransactionSynchronizationProcessor expressionEvaluatingTransactionSynchronizationProcessor =
        new ExpressionEvaluatingTransactionSynchronizationProcessor();
	expressionEvaluatingTransactionSynchronizationProcessor.setAfterCommitExpression(parser.parseExpression("@syncProcessor.process(payload)"));
	var transactionSynchronizationFactory = new DefaultTransactionSynchronizationFactory(expressionEvaluatingTransactionSynchronizationProcessor);
...
----
@Bean
fun imapIdleFlow(imapMailReceiver: ImapMailReceiver, receiveChannel: MessageChannel,
    mailMessageHandler: MailMessageHandler): IntegrationFlow {
    val processor = ExpressionEvaluatingTransactionSynchronizationProcessor().apply {
        setAfterCommitExpression(
            SpelExpressionParser().parseExpression("@syncProcessor.process(payload)")
        )
    }
    ...
    return IntegrationFlow
        .from(Mail.imapIdleAdapter(imapMailReceiver)
            .autoStartup(true)
            .id("kotlinImapIdleAdapter")
            .transactionSynchronizationFactory(DefaultTransactionSynchronizationFactory(processor)))
        .channel(receiveChannel)
        .handle(mailMessageHandler)
        .get()
}
...
----
@Bean
IntegrationFlow imapIdleFlow(ImapMailReceiver imapMailReceiver, MessageChannel receiveChannel,
    MailMessageHandler mailMessageHandler) {

    def processor = new ExpressionEvaluatingTransactionSynchronizationProcessor()
    processor.afterCommitExpression = new SpelExpressionParser().parseExpression("@syncProcessor.process(payload)")
    def transactionSynchronizationFactory =  new DefaultTransactionSynchronizationFactory(processor)

    return IntegrationFlow.from(
        Mail.imapIdleAdapter(imapMailReceiver)
            .autoStartup(true)
            .id("groovyImapIdleAdapter")
            .transactionSynchronizationFactory(transactionSynchronizationFactory)
    )
...
To configure a mail component in Java DSL, the framework provides a `o.s.i.mail.dsl.Mail` factory, which can be used like this:
