...
* `keepWorkingDirectory` - specifies whether a generated target zip file name must remain under the configured `workDirectory` when the result type is `FILE`.
Default is `false`.
For XML configuration, use the `keep-working-directory` attribute.
When set to `true`, generated names that resolve outside the work directory (for example, with `..`) are rejected.
