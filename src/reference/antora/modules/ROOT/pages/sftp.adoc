...
The most drastic change has happened to the `DefaultSftpSessionFactory`, which is now based on the `org.apache.sshd.client.SshClient` and exposes several of its configuration properties.
