...
The `ZipTransformer` now provides a `keepWorkingDirectory` option to reject generated zip file names that resolve outside the configured `workDirectory` for `FILE` results.
See xref:zip.adoc[] for more information.
