...
The Redis module now provides a Java DSL API via its dedicated `org.springframework.integration.redis.dsl.Redis` factory. See xref:redis.adoc#redis-inbound-channel-adapter[Inbound Channel Adapter] and xref:redis.adoc#redis-outbound-channel-adapter[Outbound Channel Adapter] for more details.
