...
The `JdbcChannelMessageStore` now supports Jackson JSON serialization as an alternative
to Java serialization. New components `JacksonChannelMessageStorePreparedStatementSetter`
and `JacksonMessageRowMapper` enable storing messages in JSON format.
This requires modifying the database schema to use text-based column types
(such as JSONB, JSON, TEXT, or CLOB) instead of binary types.
