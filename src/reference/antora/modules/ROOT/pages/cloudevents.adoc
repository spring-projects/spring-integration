...
When you specify an `EventFormat` or `eventFormatContentTypeExpression`, the transformer uses the `EventFormat` to generate the `CloudEvents` in the payload.
...
This transformer supports the CloudEvents specification v1.0 and processes CloudEvents from two payload types: `CloudEvents` objects or serialized `CloudEvents` byte arrays.

The transformer extracts `CloudEvents` data from the message payload and maps `CloudEvents` attributes along with `CloudEvents` extensions to message headers with a `ce-` prefix.
...
When the message payload is a `CloudEvents` instance, the transformer:
...
4. Preserves all original message headers unless a header key matches a `CloudEvents` attribute or extension, in which case the original value is overwritten.
...
When the message payload is a `byte[]` containing a serialized `CloudEvents`, the transformer:

1. Uses the `content-type` header to resolve the appropriate `EventFormat` via `EventFormatProvider`.
2. Deserializes the payload to a `CloudEvents` object using the resolved format.
...
The transformer maps `CloudEvents` attributes to message headers using the following `CloudEventHeaders` constants:
