...
NOTE: `FromCloudEventTransformer` allows the user to set an `EventFormat` that will be used if the `EventFormatProvider` fails to find an `EventFormat` for the `content-type` header or if the message does not contain a `content-type` header.
