...
----
The `outputMessage` from the example above should look something like:
```
GenericMessage [
    payload = byte[13],
    headers = {
        ...
        id         = 1004c34e-adc7-1c9a-35e7-a06c0aea6e25,
        ce-type    = order.created,
        ce-id      = event-123,
        timestamp  = 1770391954099
    }
]
```
...
3. Then follows the same steps enumerated in the <<cloudeventobjectype>> section.

Information on supported content types are discussed in the <<eventformats>> section.

NOTE: `FromCloudEventTransformer` allows the user to set an `EventFormat` that will be used if the `EventFormatProvider` fails to find an `EventFormat`.
