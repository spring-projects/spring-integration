...
To begin interacting with Redis, a connection must be obtained first.
...
Since Spring Integration 3.0, the Redis module has provided an implementation of `IntegrationEvent`, which, in turn, is a `org.springframework.context.ApplicationEvent`.
...
As described in the _Enterprise Integration Patterns_ (EIP) book, a https://www.enterpriseintegrationpatterns.com/MessageStore.html[message store] lets persist messages.
...
----
@Bean
SourcePollingChannelAdapterFactoryBean storeInboundChannelAdapter(RedisConnectionFactory redisConnectionFactory) {
    ...
    var adapter = new SourcePollingChannelAdapterFactoryBean();
    adapter.setSource(messageSource);
    adapter.setOutputChannel(MessageChannels.queue("redisChannel").getObject());
    adapter.setPollerMetadata(Pollers.fixedDelay(2000).getObject());
    return adapter;
}
----
...
SourcePollingChannelAdapterFactoryBean storeInboundChannelAdapter(RedisConnectionFactory redisConnectionFactory,
    ...
    var adapter = new SourcePollingChannelAdapterFactoryBean();
    adapter.setSource(messageSource);
    adapter.setOutputChannel(MessageChannels.queue("storeInboundChannelAdapterOutputChannel").getObject());
    adapter.setPollerMetadata(Pollers
            .fixedDelay(1000)
            .transactional(new PseudoTransactionManager())
            .transactionSynchronizationFactory(syncFactory)
            .getObject());
...
Spring Integration 4.0 introduced the Redis command gateway to let perform any standard Redis command by using the generic `RedisConnection#execute` method.
