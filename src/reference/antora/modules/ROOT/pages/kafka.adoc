...
----
@Bean
fun shareListenerFromKafkaFlow() =
    integrationFlow(
        Kafka.shareMessageDrivenChannelAdapter(shareConsumerFactory(), "someTopic")
            .configureListenerContainer {
                it.shareAckMode(ContainerProperties.ShareAckMode.MANUAL)
                it.concurrency(3)
                it.id("someTopicContainer")
            }) {
        handle { m ->
            val ack = m.headers[KafkaHeaders.ACKNOWLEDGMENT] as ShareAcknowledgment
            // process the payload...
            ack.acknowledge()
        }
    }
...
@Bean
fun shareConsumerFactory(): ShareConsumerFactory<String, String> {
    val props = mapOf(
        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG to brokerAddress,
        ConsumerConfig.GROUP_ID_CONFIG to "someGroup")
    // set more properties
    return DefaultShareConsumerFactory(props)
}
----
...
shareListenerFromKafkaFlow() {
    integrationFlow(
        ...
        handle { m ->
            ShareAcknowledgment ack = m.headers[KafkaHeaders.ACKNOWLEDGMENT]
            // process the payload...
            ack.acknowledge()
        }
    }
...
shareConsumerFactory() {
    ...
    props[ConsumerConfig.GROUP_ID_CONFIG] = 'someGroup'
    // set more properties
