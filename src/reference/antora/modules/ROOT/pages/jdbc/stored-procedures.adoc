...
The stored procedure components provide full XML Namespace support, and configuring the components is the same as for the general purpose JDBC components discussed earlier.
...
This section contains two examples that call stored procedures.
...
The Spring Integration Samples project provides an https://github.com/spring-projects/spring-integration-samples/tree/main/intermediate/stored-procedures-oracle[example] of using Oracle stored procedures.
=====
...
The following listing shows a Java method that can back such a procedure (for example as an H2 alias):

[source,java]
----
public static ResultSet findAllCoffeeBeverages()
            throws SQLException {
    ...
}
----
...
[source,java]
----
public static String findCoffee(int coffeeId)
            throws SQLException {
    ...
}
----
