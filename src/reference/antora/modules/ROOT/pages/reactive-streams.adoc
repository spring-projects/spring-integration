...
When the https://github.com/micrometer-metrics/context-propagation[Context Propagation] library is on the classpath, the Project Reactor can take `ThreadLocal` values (e.g. https://docs.micrometer.io/micrometer/reference/observation/introduction.html[Micrometer Observation] or `SecurityContextHolder`) and store them into a `Subscriber` context.
