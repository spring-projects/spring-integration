...
public class R2dbcMessageHandler extends AbstractReactiveMessageHandler {
	...
	private Mono<Void> handleDelete(Message<?> message) {
		if (this.tableNameExpression != null) {
			...
			DatabaseClient.DeleteMatchingSpec deleteSpec = this.r2dbcEntityOperations.getDatabaseClient().delete().from(tableName);
			return deleteSpec.matching(criteria).then();
		}
		else {
	...
	private Mono<Void> handleUpdate(Message<?> message) {
		if (this.tableNameExpression != null) {
			...
			Criteria criteria = evaluateCriteriaExpression(message);
			DatabaseClient.GenericUpdateSpec updateSpec = this.r2dbcEntityOperations.getDatabaseClient().update()
					.table(tableName);
			return updateSpec.using(Update.from(updateMap)).matching(criteria).then();
		}
		else {
	...
	private Mono<Void> handleInsert(Message<?> message) {
		...
		if (this.tableNameExpression != null) {
			...
			String tableName = evaluateTableNameExpression(message);
			Map<String, Object> values = evaluateValuesExpression(message);
			DatabaseClient.GenericInsertSpec<Map<String, Object>> insertSpec = this.r2dbcEntityOperations.getDatabaseClient()
					.insert()
					.into(tableName);
	...
	private Map<String, Object> evaluateValuesExpression(Message<?> message) {
		Map<String, Object> fieldValues =
				this.valuesExpression.getValue(this.evaluationContext, message, Map.class);
