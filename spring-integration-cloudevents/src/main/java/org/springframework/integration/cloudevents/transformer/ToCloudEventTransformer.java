...
/**
 * Transform messages to {@link CloudEvent} format with attributes and extensions mapping.
 ...
 *   into a message where {@link CloudEvent} attributes are mapped to headers with a configurable
 *   prefix (default: {@link CloudEventHeaders#PREFIX}), and the {@link CloudEvent} data becomes the payload.
...
public class ToCloudEventTransformer extends AbstractTransformer {
	...
	private String cloudEventPrefix = CloudEventHeaders.PREFIX;
	...
	/**
	 * Set the {@link Expression} to create {@link CloudEvent} {@code id}.
	 * <p>Default is to extract the {@code id} from the {@link MessageHeaders} of the message.
	 * @param eventIdExpression the expression to create the {@code id} for each {@link CloudEvent}
	 */
	...
	/**
	 * Set the {@link Expression} to create {@link CloudEvent} {@code source}.
	 * <p>Default is {@code "/spring/" + appName + "." + getBeanName())}.
	 * @param sourceExpression the expression to create the {@code source} for each {@link CloudEvent}
	 */
	...
	/**
	 * Set the {@link Expression} to extract the {@code type} for the {@link CloudEvent}.
	 * <p>Default is {@code spring.message}.
	 * @param typeExpression the expression to create the {@code type} for each {@link CloudEvent}
	 */
	...
	/**
	 * Set the {@link Expression} to create the {@code dataSchema} for the {@link CloudEvent}.
	 * @param dataSchemaExpression the expression to create the {@code dataSchema} for each {@link CloudEvent}
	 */
	...
	/**
	 * Set the {@link Expression} to create the {@code subject} for the {@link CloudEvent}.
	 * @param subjectExpression the expression to create the {@code subject} for each {@link CloudEvent}
	 */
	...
	/**
	 * Set the {@link EventFormat} to use for {@link CloudEvent} serialization.
	...
	/**
	 * Set the prefix for {@link CloudEvent} headers in binary content mode.
	...
	/**
	 * Custom {@link CloudEvent} extension implementation that wraps a map of headers
	 * as {@link CloudEvent} extension attributes.
	 */
	...
	/**
	 * A {@link CloudEventWriter} implementation that converts {@link CloudEvent} objects into
	 * Spring Integration {@link Message} instances with {@link CloudEvent} attributes as headers.
	 */
	private static class MessageBuilderMessageWriter implements CloudEventWriter<Message<byte[]>>,
		...
		/**
		 * Initialize the writer with the {@link CloudEvent} specification version.
		 * Set the specification version as a message header using the configured version key.
		 * @param version the {@link CloudEvent} specification version
		...
		/**
		 * Complete the message creation with {@link CloudEvent} data.
		 * @param value the {@link CloudEvent} data to use as the message payload
		 * @return the Spring Integration message with {@link CloudEvent} data and attributes
		...
		/**
		 * Complete the message creation without {@link CloudEvent} data.
		 * Create a message with an empty payload when the {@link CloudEvent} contains no data.
		 * @return the Spring Integration message with an empty payload and {@link CloudEvent} attributes as headers
		 */
		@Override
